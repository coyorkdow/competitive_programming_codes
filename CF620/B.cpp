...
int main() {
  ...
  string ans, mid;
  while (!cnt.empty()) {
    string cur = cnt.begin()->first;
    string rev = cur;
    reverse(all(rev));
    if (cur == rev) {
      for (int i = 0; i < cnt.begin()->second / 2; i++) ans += rev;
      if (cnt.begin()->second & 1) {
        if (rev.size() > mid.size()) mid = rev;
      }
    } else if (cnt.count(rev)) {
      int num = min(cnt.begin()->second, cnt[rev]);
      for (int i = 0; i < num; i++) ans += cur;
    }
    cnt.erase(cur);
    cnt.erase(rev);
  }
  if (ans.empty() && mid.empty()) {
    cout << 0 << endl;
  } else {
