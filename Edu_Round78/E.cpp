...
int main() {
  ...
  vector<int> siz(n, 1);
  vector<pii> ans(n);
  function<int(int, int)> cal_size = [&](int cur, int p) -> int {
    for (auto &i : G[cur]) {
      if (i == p) continue;
      siz[cur] += cal_size(i, cur);
    }
    return siz[cur];
  };
  function<void(int, int, int, int)> get = [&](int cur, int p, int bg, int ed) {
    ans[cur].first = bg;
    ans[cur].second = ed - (siz[cur] - 1) * 2 + (G[cur].size() - 1);
    if (p == -1) ans[cur].second++;
    int dis = G[cur].size() - 1;
    if (p == -1) dis++;
    for (auto &i : G[cur]) {
      if (i == p) continue;
      get(i, cur, ans[cur].second - dis, ed);
      dis--;
      ed -= siz[i] * 2 - 1;
    }
  };
