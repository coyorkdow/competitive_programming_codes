...
template <>
istream &operator>><pair<ll, ll>>(istream &is, vector<pair<ll, ll>> &v) {
  for (pair<ll, ll> &x : v) is >> x.first >> x.second;
  return is;
}
template <class T>
...
int main() {
  ...
  int n;
  cin >> n;
  vector<pll> arr(n), lines;
  cin >> arr;
  for (int i = 0; i < n; i++) {
    lines.push_back(pll(arr[i].first - arr[(i + 1) % n].first,
                        arr[i].second - arr[(i + 1) % n].second));
  }
  auto check = [](pll x, pll y) -> bool {
  ...
  for (int i = 0; i < n / 2; i++) {
    if (check(lines[i], lines[i + n / 2])) continue;
    cout << "NO\n";
    exit(0);
  }
  cout << "YES\n";
