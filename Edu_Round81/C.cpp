...
int main() {
  ...
  while (T--) {
    ...
    for (int i = s.size() - 1; i >= 0; i--) {
      for (int j = 0; j < 26; j++) nxt[i + 1][j] = last[j];
      last[s[i] - 'a'] = i + 1;
    }
    for (int j = 0; j < 26; j++) nxt[0][j] = last[j];
    int ptr = -1, ans = 0;
    while (ptr < (int)t.size() - 1) {
      int i = nxt[0][t[ptr + 1] - 'a'];
      if (i == -1) GG;
      ans++, ptr++;
      if (ptr == t.size() - 1) break;
      while (ptr < t.size() - 1 && nxt[i][t[ptr + 1] - 'a'] != -1) {
        i = nxt[i][t[ptr + 1] - 'a'];
        ptr++;
      }
    }
