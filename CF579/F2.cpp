...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    if (arr[i].second >= 0) {
      if (arr[i].first <= res) res += arr[i].second, ans++;
      arr[i] = {1e9, 1e9};
    }
  }
  sort(all(arr));
  arr.resize(distance(arr.begin(), lower_bound(all(arr), pii(1e9, 1e9))));
  if (arr.back() == pii(1e9, 1e9)) arr.pop_back();
  ...
  reverse(all(arr));
  vector<int> dp(res + 1, -1e9);
  dp[res] = ans;
  for (auto& i : arr) {
    for (int j = max(i.first, -i.second); j <= res; j++)
      chkmax(dp[j + i.second], dp[j] + 1);
  }
  cout << accumulate(all(dp), 0, [](int a, int b) { return max(a, b); })
       << endl;
  return 0;
}