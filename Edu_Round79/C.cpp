...
int main() {
  ...
  while (T--) {
    int n, m;
    cin >> n >> m;
    vector<int> gift_no(n), send_ord(m);
    cin >> gift_no >> send_ord;
    vector<int> stack_dep(n);
    for (int i = 0; i < n; i++) stack_dep[gift_no[i] - 1] = i + 1;
    ll ans = 0;
    for (int i = 0; i < m; i++) {
      ans += (stack_dep[send_ord[i] - 1] - i) * 2 - 1;
      int j;
      for (j = i + 1; j < m; j++) {
        if (stack_dep[send_ord[j] - 1] < stack_dep[send_ord[i] - 1])
          ans++;
        else
          break;
      }
