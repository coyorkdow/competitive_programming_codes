...
int main() {
  ...
  while (T--) {
    ...
    if (a == b) {
      ans = min(ans, abs(a + 1 - (c - 1)) + abs(b + 1 - (c - 1)));
      ans = min(ans, abs(a + 1 - c) + abs(b + 1 - c));
    } else if (b == c) {
      ans = min(ans, abs(b - 1 - (a + 1)) + abs(c - 1 - (a + 1)));
      ans = min(ans, abs(b - 1 - a) + abs(c - 1 - a));
    } else {
      ans = min(ans, abs(a + 1 - b) + abs(a + 1 - (c - 1)) + abs(b - (c - 1)));
      ans = min(ans, abs(a + 1 - (b - 1)) + abs(a + 1 - (c - 1)) +
                         abs(b - 1 - (c - 1)));
      ans = min(ans, abs(a + 1 - (b + 1)) + abs(a + 1 - (c - 1)) +
                         abs(b + 1 - (c - 1)));
    }
