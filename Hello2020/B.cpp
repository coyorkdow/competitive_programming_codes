...
int main() {
  ...
  auto check = [&](vector<int> &arr, int &maxi, int &mini) -> bool {
    maxi = 0, mini = 1e9;
    bool flag = false;
    for (int i = 0; i < arr.size(); i++) {
      if (i && arr[i] > arr[i - 1]) flag = true;
      amax(maxi, arr[i]), amin(mini, arr[i]);
    }
    return flag;
  };
  vector<bool> flag(n);
  ...
  for (int i = 0; i < n; i++) {
    flag[i] = check(arrs[i], maxi[i], mini[i]);
    if (flag[i]) maxi[i] = 1e9;
  }
  sort(all(maxi));
  ll res = 0;
  for (int i = 0; i < n; i++) {
    if (flag[i]) continue;
    res += upper_bound(all(maxi), mini[i]) - maxi.begin();
  }
