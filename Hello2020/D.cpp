...
int main() {
  ...
  struct ele {
    ...
    bool operator<(const ele &__y) const {
      if (t != __y.t) return t < __y.t;
      return (int)type < (int)__y.type;
    }
  };
  auto check = [&]() -> bool {
    vector<ele> E;
    for (int i = 0; i < n; i++) {
      E.push_back(ele{B[i].first, B[i].second, A[i].first, true});
      E.push_back(ele{B[i].first, B[i].second, A[i].second + 1, false});
    }
    multiset<int> l, r;
    sort(all(E));
    for (auto &i : E) {
      if (i.type) {
        int _min = i.r;
        if (!r.empty()) amin(_min, *r.begin());
        int _max = i.l;
        if (!l.empty()) amax(_max, *l.rbegin());
        if (_max > _min) return false;
        l.insert(i.l);
        r.insert(i.r);
      } else {
        l.erase(l.lower_bound(i.l));
        r.erase(r.lower_bound(i.r));
      }
    }
    return true;
  };
