...
struct trie {
  ...
  int num, ans;
  trie() : tree(vector<vector<int>>(5e6, vector<int>(2))), num(0), ans(INT_MAX) {}
  void insert(const string &s) {
    int cur = 0;
    for (auto &i : s) {
      if (!tree[cur][i - '0']) tree[cur][i - '0'] = ++num;
      cur = tree[cur][i - '0'];
    }
  }
  void dfs(int node, int bit, int res = 0) {
    if (bit == -1) {
      ans = min(ans, res);
      return;
    }
    if (tree[node][0] && tree[node][1]) {
      dfs(tree[node][0], bit - 1, res | (1 << bit));
      dfs(tree[node][1], bit - 1, res | (1 << bit));
    } else
      dfs(tree[node][0] | tree[node][1], bit - 1, res);
  }
};
int main() {
  quickio;
