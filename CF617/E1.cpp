...
int main() {
  ...
  string str;
  cin >> str;
  vector<int> cols(n), w(26);
  for (int i = 0; i < n; i++) {
    cols[i] = 1 + accumulate(w.begin() + str[i] - 'a' + 1, w.end(), 0,
                             [](int x, int y) { return max(x, y); });
    w[str[i] - 'a'] = cols[i];
  }
  // cout << accumulate(all(cols), 0, [](int x, int y) { return max(x, y); })
  //      << endl;
  // cout << cols << endl;
  if (accumulate(all(cols), 0, [](int x, int y) { return max(x, y); }) <= 2) {
    cout << "YES\n";
    for (int i = 0; i < n; i++)
      if (cols[i] == cols[0])
        cout << 0;
