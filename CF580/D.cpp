...
int main() {
  ...
  sort(all(arr), greater<ull>{});
  arr.resize(distance(arr.begin(), lower_bound(all(arr), 0, greater<ull>{})));
  // cout << arr << endl;
  n = arr.size();
  vector<vector<ll>> G(n, vector<ll>(n, 1e9));
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (i != j && arr[i] & arr[j]) {
        G[i][j] = 1;
  ...
  for (int k = 0; k < n; k++) {
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i != j) chkmin(ans, dis[i][j] + G[i][k] + G[k][j]);
      }
    }
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
  ...
  // for (auto &each : dis) cout << each << endl;
  if (ans == 1e9) ans = -1;
  cout << ans << endl;
  return 0;
}
