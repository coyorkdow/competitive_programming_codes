...
int main() {
  ...
  for (int j = 0; j < n; j++) {
    for (int k = n - 1; k >= j; k--) {
      dp[0][j][k] = 1;
      if (k != n - 1) sum[0][j][k] += sum[0][j][k + 1];
      sum[0][j][k] = (sum[0][j][k] + dp[0][j][k]) % mod;
    }
  }
  for (int i = 1; i < m; i++) {
    ...
    for (int j = 0; j < n; j++) {
      for (int k = n - 1; k >= j; k--) {
        if (j)
          dp[i % 2][j][k] = dp[i % 2][j - 1][k] + sum[(i - 1) % 2][j][k];
        else
          dp[i % 2][j][k] = sum[(i - 1) % 2][j][k];
        dp[i % 2][j][k] %= mod;
        if (k != n - 1) sum[i % 2][j][k] += sum[i % 2][j][k + 1];
        sum[i % 2][j][k] = (sum[i % 2][j][k] + dp[i % 2][j][k]) % mod;
      }
    }
  }
  ll ans = 0;
  for (int i = 0; i < n; i++) ans = (ans + sum[(m - 1) % 2][i][i]) % mod;
  cout << ans << endl;
  return 0;
}